...
static volatile SI_SEGMENT_VARIABLE(TxBuffer[SPI_BUF_SIZE], uint8_t, EFM8PDL_SPI0_TX_SEGTYPE);
...
void SPI_StartTransfer(uint8_t* buffer, uint8_t size)
{
  ...
  TMR2 = -(DELAY_TIMER_FREQ / 1000000 * SPI_CS_SETUP_US);
...
void SPI0_transferCompleteCb(void)
{
  ...
  TMR2 = -(DELAY_TIMER_FREQ / 1000000 * SPI_CS_HOLD_US);
  TMR2CN0 |= TMR2CN0_TR2__BMASK;
